...
Imports System.Data
Public Class frmServistaSearch
    ...
    Public Sub New(ByVal sender As Object, ByVal formTag As String, Optional ByVal OverrideTblName As String = "")
        ...
        ElseIf TypeOf callingFormControl Is DataGridViewTextBoxCell Then
            ...
            controlName = callingFormControl.OwningColumn.Name.ToString '//sets the selected row item to the gridview cell
    ...
    Private Sub search()
        ServistaSqlConnection.ConnectionString = ServistaConnectionString
        ServistaSqlConnection.Open()
        ...
        If Not txtSearch.Text.Length = 0 Or viewMode = 1 Then '//Disables searching if no text is entered, but provides a exception when show all record view mode is enabled
            Try
                ...
                ServistaCommand = New SqlCommand(strSelectQuery, ServistaSqlConnection)
                ServistaCommand.Parameters.AddWithValue("@searchText", Trim(txtSearch.Text))
                recordCount = dbConnections.ServistaCommand.ExecuteScalar
                ...
                If recordCount >= 0 Then '//If there are records to be searched
                    ...
                    End If
                    ServistaCommand = New SqlCommand(strSelectQuery, ServistaSqlConnection)
                    ' ServistaCommand.CommandText = strSelectQuery
                    ServistaCommand.Parameters.AddWithValue("@searchText", Trim(txtSearch.Text))
                    ServistaDset = New DataSet
                    ServistasqlAdeptor = New SqlDataAdapter(ServistaCommand)
                    ServistasqlAdeptor.Fill(ServistaDset, "tmpResults")
                    Dim colWidthCount As Integer = 0
                    dgSearch.DataSource = ServistaDset.Tables("tmpResults")
    ...
    Private Sub dgSearch_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles dgSearch.KeyDown
        ...
        If e.KeyCode = Keys.Enter And recordCount > 0 Then '//On cell selection made by enter
            ...
            ElseIf TypeOf callingFormControl Is DataGridViewTextBoxCell Then
                callingFormControl.Value = selectedRowItem '//sets the selected row item to the gridview cell
            End If
            Me.Close()
        End If
