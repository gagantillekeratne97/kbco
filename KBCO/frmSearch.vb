...
Public Class frmSearch
    ...
    Private querySplit() As String '//Full query splitted into two parts
    ...
    Public Sub New(ByVal sender As Object, ByVal formTag As String, Optional ByVal OverrideTblName As String = "")
       ...
       connectionStaet()
        ...
        Else 'If TypeOf callingFormControl Is DataGridViewTextBoxCell Then

            controlName = callingFormControl.CurrentCell.OwningColumn.Name.ToString '//sets the selected row item to the gridview cell
            Me.Text = "Search - " & callingFormControl.CurrentCell.OwningColumn.HeaderText.ToString
        End If
        'Me.Text = "Search - " & sender.FindForm.Text
        '//Select search controls from the db
        Try
            strSelectQuery = "SELECT CUSTOM_SQL,DEFAULT_SEARCHBY,RETURN_FIELD,DEFAULT_VIEW_MODE,RESULTS_PER_PAGE,COLUMN_SIZES,ORDER_BY,DEFAULT_ORDER_BY FROM U_TBLSEARCH_CONTROLS WHERE FORM_TAG='" & formTag & "' AND CONTROL_NAME='" & controlName & "' AND COM_ID = '" & globalVariables.selectedCompanyID & "'"
    ...
    Private Sub search()
        If Not txtSearch.Text.Length = 0 Or viewMode = 1 Then '//Disables searching if no text is entered, but provides a exception when show all record view mode is enabled
            Try
                ...
                dbConnections.sqlCommand = New SqlCommand(strSelectQuery, dbConnections.sqlConnection)
                dbConnections.sqlCommand.Parameters.AddWithValue("@searchText", Trim(txtSearch.Text))
                recordCount = dbConnections.sqlCommand.ExecuteScalar
                ...
                If recordCount >= 0 Then '//If there are records to be searched
                    ...
                    End If
                    dbConnections.sqlCommand.CommandText = strSelectQuery
                    dbConnections.dset = New DataSet
                    dbConnections.sqlAdapter = New SqlDataAdapter(sqlCommand)
                    dbConnections.sqlAdapter.Fill(dset, "tmpResults")
                    Dim colWidthCount As Integer = 0
                    dgSearch.DataSource = dset.Tables("tmpResults")
    ...
    Private Sub dgSearch_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles dgSearch.KeyDown
        ...
        If e.KeyCode = Keys.Enter And recordCount > 0 Then '//On cell selection made by enter
            ...
            Else 'If TypeOf callingFormControl Is DataGridViewTextBoxCell Then
                ...
                callingFormControl.CurrentCell.Value = selectedRowItem '//sets the selected row item to the gridview cell '//sets the selected row item to the gridview cell
    ...
    Private Sub DatagrdAssendingorder(ByRef datagridName As DataGridView)
        ...
        If datagridName.RowCount = 0 Then
            Exit Sub
        End If
    ...
    Private Sub dgSearch_MouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles dgSearch.MouseDoubleClick
        ...
        If TypeOf callingFormControl Is System.Windows.Forms.TextBox Then
            callingFormControl.Text = selectedRowItem '//sets the selected row item to the txtBox
        Else 'If TypeOf callingFormControl Is DataGridViewTextBoxCell Then
            callingFormControl.CurrentCell.Value = selectedRowItem '//sets the selected row item to the gridview cell
        End If
        Me.Close()
