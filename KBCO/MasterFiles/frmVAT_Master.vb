...
#Region "Add/ Save/Delete"
        ...
        If conf = vbYes Then
            ...
            Try
                ...
                If isEditClicked Then
                    errorEvent = "Edit"
                    strSQL = "UPDATE    MTBL_VAT_MASTER SET VAT_DESC =@VAT_DESC, VAT_PRE =@VAT_PRE, MD_BY ='" & userSession & "', MD_DATE =GETDATE(),IS_NBT=@IS_NBT,IS_VAT=@IS_VAT,INV_HEADING_NAME=@INV_HEADING_NAME WHERE     (COM_ID =@COM_ID) AND (VAT_TYPE_ID = @VAT_TYPE_ID)"
                Else
                    errorEvent = "Save"
                    strSQL = "INSERT INTO MTBL_VAT_MASTER  (COM_ID, VAT_TYPE_ID, VAT_DESC, VAT_PRE, CR_BY, CR_DATE,IS_NBT,IS_VAT,INV_HEADING_NAME) VALUES     (@COM_ID, @VAT_TYPE_ID, @VAT_DESC, @VAT_PRE, '" & userSession & "', GETDATE(),@IS_NBT,@IS_VAT,@INV_HEADING_NAME)"
                End If
                ...
                dbConnections.sqlCommand.Parameters.AddWithValue("@VAT_TYPE_ID", Trim(txtVatTypeID.Text))
                dbConnections.sqlCommand.Parameters.AddWithValue("@VAT_DESC", Trim(txtVatTypeName.Text))
                dbConnections.sqlCommand.Parameters.AddWithValue("@VAT_PRE", Trim(txtVatPre.Text))
                ...
                dbConnections.sqlCommand.Parameters.AddWithValue("@INV_HEADING_NAME", Trim(txtInvoiceHeadingName.Text))

                If dbConnections.sqlCommand.ExecuteNonQuery() Then save = True Else save = False

                If save = True Then
                    If isEditClicked Then
                        AuditDelete(Me.Text, userSession, userName, txtVatTypeID.Text, txtVatTypeName.Text + "(Edit)")
                    Else
                        AuditDelete(Me.Text, userSession, userName, txtVatTypeID.Text, txtVatTypeName.Text + "(Saved)")
                    End If
    ...
    Private Function delete() As Boolean
        ...
        Try
            Dim IDbackup As String = Trim(txtVatTypeID.Text)
            Dim IDdesc As String = Trim(txtVatTypeName.Text)
            connectionStaet()
            Dim confDelete = MessageBox.Show("" & DeleteMessage & "" & txtVatTypeName.Text, "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2)
            If confDelete = vbYes Then
                strSQL = "DELETE FROM MTBL_VAT_MASTER WHERE     (COM_ID = @COM_ID) AND (VAT_TYPE_ID = @VAT_TYPE_ID)"
    ...
    Private Sub FormEdit()
        ...
        If conf = vbYes Then
            ...
            cbNBT.Enabled = True
            cbVAT.Enabled = True
            txtInvoiceHeadingName.Enabled = True
...
#End Region
    ...
    '===================================================================================================================
#Region "Form Events"
    Private Sub frmVAT_Master_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Enter
        isFormFocused = True
    End Sub

    Private Sub frmVAT_Master_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        globalFunctions.globalButtonActivation(False, False, False, False, False, False)

    End Sub

    Private Sub frmVAT_Master_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub frmVAT_Master_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        DisableALLTextBoxSound(Me, e)
        If e.KeyCode = Keys.Enter Then SendKeys.Send("{TAB}")
    End Sub

    Private Sub frmVAT_Master_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Leave
        isFormFocused = False
    End Sub

    Private Sub frmVAT_Master_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Me.Load
        FormClear()
    End Sub

    Private Sub frmVAT_Master_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
...
#Region "Validations"
    Private Function isDataValid()
        isDataValid = False
        If generalValObj.isPresent(txtVatTypeID) = False Then
            Exit Function
        End If
        If generalValObj.isPresent(txtVatTypeName) = False Then
            Exit Function
        End If
        If generalValObj.isPresent(txtVatPre) = False Then
            Exit Function
        End If
        If generalValObj.isPresent(txtInvoiceHeadingName) = False Then
            Exit Function
        End If
        isDataValid = True
        Return isDataValid
    ...
    Private Sub FormClear()
        ...
        txtVatTypeName.Text = ""
        txtVatPre.Text = ""
        txtInvoiceHeadingName.Text = ""
        txtVatTypeID.Enabled = True
        txtVatTypeName.Enabled = True
        txtVatPre.Enabled = True
        cbNBT.Checked = False
        txtInvoiceHeadingName.Enabled = True
...
#Region "Text Box events"
    ...
    Private Sub txtVatTypeID_KeyDown(sender As Object, e As KeyEventArgs) Handles txtVatTypeID.KeyDown
        If e.KeyCode = Keys.F2 Then Dim searchObj As New frmSearch(sender, Me.Tag)
    End Sub


    Private Sub txtVatTypeID_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtVatTypeID.KeyPress
        generalValObj.isDigit(e)
    End Sub
    Private Sub txtVatPre_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtVatPre.KeyPress
        generalValObj.isNumericWithDecimals(sender, e)
    End Sub


    Private Sub txtVatTypeID_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles txtVatTypeID.Validating
        ...
        connectionStaet()
        Try
            strSQL = "SELECT       VAT_DESC, VAT_PRE,IS_NBT,IS_VAT,INV_HEADING_NAME FROM MTBL_VAT_MASTER WHERE COM_ID = @COM_ID and VAT_TYPE_ID = @VAT_TYPE_ID"
            dbConnections.sqlCommand = New SqlCommand(strSQL, dbConnections.sqlConnection)
            dbConnections.sqlCommand.Parameters.AddWithValue("@COM_ID", globalVariables.selectedCompanyID)
            dbConnections.sqlCommand.Parameters.AddWithValue("@VAT_TYPE_ID", Trim(txtVatTypeID.Text))
            ...
            While dbConnections.dReader.Read
                hasRecords = True
                txtVatTypeName.Text = dbConnections.dReader.Item("VAT_DESC")
                txtVatPre.Text = dbConnections.dReader.Item("VAT_PRE")
                ...
                Else
                    ...
                    End If
                End If
                If IsDBNull(dbConnections.dReader.Item("INV_HEADING_NAME")) Then
                    txtInvoiceHeadingName.Text = ""
                Else
                    txtInvoiceHeadingName.Text = dbConnections.dReader.Item("INV_HEADING_NAME")
                End If


                txtVatTypeID.Enabled = False
                txtVatTypeName.Enabled = False
                txtVatPre.Enabled = False
                cbNBT.Enabled = False
                cbVAT.Enabled = False
                txtInvoiceHeadingName.Enabled = False
            End While
            dbConnections.dReader.Close()
            If hasRecords Then
...
#End Region
    ...
    Private Sub txtVatTypeID_TextChanged(sender As Object, e As EventArgs) Handles txtVatTypeID.TextChanged

    End Sub

    Private Sub txtVatTypeID_TabIndexChanged(sender As Object, e As EventArgs) Handles txtVatTypeID.TabIndexChanged
